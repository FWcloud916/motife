...
The delivery *form* is decided (2026-08-19): a single-binary native application plus a web interface, in parallel — not an npm package. Getting there is Phase 7, gated on the Phase 5–6 Rust migration; FFmpeg packaging (static link / embed / require a system install) and WASM-vs-HTTP-server for the web side are the two open technical decisions inside it. The *distribution channel* (open-source tool / demo site / self-use) remains an explicit open decision owned by Phase 4 (source: motife-plan.md §3 Phase 4, Phase 7). `scripts/smoke.mjs` implements the `bundle()` → `selectComposition()` → `renderStill()` pipeline (`renderMedia()` is not used there — the smoke test renders sampled stills, not video); the critique loop's TypeScript sibling of that pipeline is `src/agent/render.ts`.
