...
- MUST NOT introduce a second render target before Phase 4 product validation is complete. The Rust engine is a **committed** post-Phase-4 route (Phase 6), no longer a conditional option — but the ordering constraint is unchanged and MUST NOT be short-circuited (source: motife-plan.md §2 決策5, §3 Phase 5–7, §4 風險與對策; decided 2026-08-19). It MUST be a clean-room implementation against the DSL spec — never a port of, nor written by referring to, Remotion's source (source: Remotion LICENSE.md — porting/modifying Remotion to build a derivative renderer is prohibited even under a paid license)
- The DSL MUST stay free of anything only Remotion can express — every new schema field MUST be implementable by the Phase 6 Rust engine (`vello`/`taffy`/`cosmic-text`) as well. This is what makes the Phase 5–7 migration possible at all (source: motife-plan.md §2 決策2, 決策5)
